...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated:
        ...
        paypalrestsdk.configure({
            "mode": "sandbox",
            "client_id": "AcLpnuljbct6rYweTwBA-gIJzfHmbO7pciSn7VOximHDH83bfkFZmIjMXj0aSl5nlnVt9EH6JdtyzH4m",
            "client_secret": "EDrXde8IQNEpHSjDUMdXTZ9zTZRJ3Nn614pGb5wierdmdgPsIwVXT8jKoN-87hBdAhT4YdyJCY6cNAbW"})
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_Nf5SWKvuCiSsqSGohVzjqngj"
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
    ...
    if request.user.is_authenticated():
        ...
        if processor == 'paypal':
            ...
            if payment.execute({"payer_id": payment.payer.payer_info.payer_id}):
                ...
                message = "There was a problem with the transaction.  Error: %s" % (payment.error_message)
            context = {
                'message': message
            }
